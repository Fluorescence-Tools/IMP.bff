/*
 * The rotamer layer: placing a library on a residue, screening it, and what
 * two screened libraries predict. One header (`Rotamer.h`, PRD-138) and one
 * interface file, wrapped late -- `RotamerEnsemble` derives from `States`
 * (avmodel.i) and returns `FRETPairGeometry` / `FRETPairEfficiencies`
 * (statesdistance.i), so everything here comes after both.
 *
 *  - The backbone frame, the atom selectors and the library registry (the
 *    former `RotamerSite.h`): nine multiplications a caller should not
 *    re-type, a selector grammar with its own ambiguity rules, and a JSON
 *    lookup with a cutoff-suffix grammar. `load_rotamer_library` is here;
 *    `load_protein_frames` is `HierarchyFrame.h`. The registry answers in
 ...
 *    not be Python. `RotamerEnsemble::from_site` -- place the library in the
 *    residue's backbone frame, screen it, take the chromophore centre and the
 *    transition dipole -- keeps its four kernel calls and their reshapes on
 *    the C++ side of the boundary, because the shapes are what the kernels
 *    themselves say.
 *  - The fps.json layer over the ensembles and the `RotamerFRET` driver,
 *    declared below.
