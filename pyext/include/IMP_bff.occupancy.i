 ...
 *
 * `OccupancyGrid` is the core of what `ProbeAccessibleVolumeOccupancyMap` was (PRD-137 step 5):
 * the same counts, windows and deltas over a plain sphere list. The layer's
 * `ProbeAccessibleVolumeOccupancyMap` (av.i) derives from it and adds the particle reading, so
