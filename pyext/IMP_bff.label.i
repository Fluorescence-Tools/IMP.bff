/*
 * label.py to %pythoncode: the system layer (Label, Quencher, attachment,
 * backbone frame, strip, FP domain detection).
 *
 * Uses IMP.atom, IMP.core, IMP.algebra -- all available at import IMP.bff time
 * (required modules). SWIG value types (StripSelection, PETParameters, Quencher)
 * are defined later in __init__.py and are found by Python name resolution at
 * call time. C++ free functions (backbone_atom_names, parse_strip_mask, etc.)
 * are on _IMP_bff and are aliased here.
 */

%pythoncode %{
# C++ free functions from earlier .i files
backbone_atom_names = _IMP_bff.backbone_atom_names
default_strip_mask = _IMP_bff.default_strip_mask
parse_strip_mask = _IMP_bff.parse_strip_mask
site_strip_mask = _IMP_bff.site_strip_mask
strip_pdb_lines = _IMP_bff.strip_pdb_lines
stripped_pdb_for = _IMP_bff.stripped_pdb_for
smith_waterman = _IMP_bff.smith_waterman
...
def backbone_frame(hierarchy, chain_id, resnum):
    ...
    return backbone_frame_from_coords(_xyz_particle(ca_p), _xyz_particle(n_p), _xyz_particle(c_p))
...
def select_atoms(hierarchy, mask: object):
...
def strip_hierarchy(hierarchy, mask: object, *, inplace: bool = False):
