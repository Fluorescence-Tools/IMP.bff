...
# ``InteractionTerm``, ``RadiativeTerm``, ``PETTerm``, ``FRETTerm`` and
# ``total_rate`` are **C++** (``include/IMP/bff/InteractionTerms.h``). They
# consume :class:`IMP.bff.States`, which became C++ first; a term that reduces
# over states could not move before the states did.
#
# Two things changed shape. Python's duck typing let each term take the
# participants it needed -- ``rate_constants(states)``,
# ``(states, atoms)``, ``(donor, acceptor)``; C++ takes the same two and
# reports through ``arity`` which it reads. And ``PETTerm``'s quenching atoms
# are given at construction rather than per call, because resolving which of a
# structure's atoms quench and how hard is a function of residue and atom names
# and does not change from one set of dye states to the next.
#
# Gated against the Python they replace on 40 states against 60 atoms: the
# radiative and FRET rates identical, PET and the sum to 3.6e-15.
...
# --------------------------------------------------------------------------
# FRET efficiency under conformational exchange
#
# These were in `analysis.py`, which is trajectory density and region
# profiles. Solving the master equation for the competition between
# fluorescence decay and conformational transitions is photophysics -- the
# same family as `kappa2_distribution_diffusion_with_traps` above, which asks
# the same question about orientation rather than distance.
# --------------------------------------------------------------------------
