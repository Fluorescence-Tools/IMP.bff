...
The single input format is ``fps.json`` (see :mod:`.fps_schema` for the
...
# IMP is imported unguarded on purpose. A try/except here bought nothing: the
...
class DockingParameters:
    ...
    #: AV backend for the (optional) distance-distribution recompute. IMP.bff
    #: is the only backend; "auto" and "imp-bff" are accepted for
    #: compatibility with the pre-move API.
    av_backend: str = "auto"
