...
def read_dye_forcefield_cif(path):
    """Read a force-field system from mmCIF -- :func:`IMP.bff.read_forcefield_cif`.

    Eighteen ``_ff_*`` categories, through the ``ihm`` C reader IMP already
    vendors and ``libimp_atom`` already exports -- the same parser the
    BinaryCIF trajectory reader uses, in text mode.

    Was 470 lines of Python: eighteen handler classes over ``ihm.format``,
    accumulating into a dict, then a pass resolving compact site numbers into
    ids. Gated against it on every shipped and generated system: identical in
    every field of every category, and 16x faster.

    One thing the port had to be told, because the file does not say it: the
    writer omits ``radius_A`` and ``mass_Da`` when every site shares the
    default, so absent means 1.7 A and 12 Da rather than zero. A zero radius
    makes every site a point and a zero mass makes the integrator divide by it.
    """
