/*
 * pyext/src/cgdye/sampling.py to %pythoncode: cgdye sampling.
...
def cluster_frames_leader(coords: np.ndarray, threshold: float) -> list[int]:
    ...
    if coords.shape[0] == 0:
        return []
    return [int(i) for i in _IMP_bff.cluster_frames_leader(coords, float(threshold))]
...
def assign_frames_to_clusters(coords: np.ndarray, centers: list[int]) -> np.ndarray:
    ...
    coords = np.ascontiguousarray(coords, dtype=np.float64)
    return np.asarray(
        _IMP_bff.assign_frames_to_clusters(
