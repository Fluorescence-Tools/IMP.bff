...
%shared_ptr(IMP::bff::NodeCallback)

%template(MapStringPortSharedPtr) std::map<std::string, std::shared_ptr<IMP::bff::Port>>;
%template(ListNodePtr) std::list<std::shared_ptr<IMP::bff::Node>>;
%template(MapStringDouble) std::map<std::string, double>;

%attribute(IMP::bff::Node, bool, is_valid, is_valid);
...
%extend IMP::bff::Node {
    ...
    %pythoncode %{
        ...
        def ports(self, v):
            p = dict()
            for key in v:
                if isinstance(v[key], IMP.bff.Port):
                    p[key] = v[key]
                elif isinstance(v[key], dict):
                    p[key] = IMP.bff.Port(**v[key])
                else:
                    p[key] = IMP.bff.Port(value=v[key])
            self.set_ports(p)
            ...
            for parameter_name in _sig.parameters:
                ...
                if o.default is empty_name:
                    if o.annotation is empty_type:
                        print("WARNING no type specified for %s IMP.bff.Port." % parameter_name)
                ...
                if o.name not in self.ports.keys():
                    p = IMP.bff.Port(name=o.name, value=value, is_output=False, is_reactive=reactive_inputs)
        ...
        def __setattr__(self, name, value):
            try:
                ...
                IMP.bff.MongoObject.__setattr__(self, name, value)
        ...
        def __init__(self,
            ...
            this = _IMP_bff.new_Node(*args, **kwargs)
            try:
