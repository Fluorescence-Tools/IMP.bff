...
struct CodecRegistry {
    ...
    CodecRegistry() {
        for (const Codec& codec : detail::builtin_codecs())
            codecs.emplace(codec.name, codec);
    }
};
...
bool codec_by_name(const std::string& name, Codec& out) {
    Codec c;
    if (!find_codec(name, &c)) return false;
    out = c;
    return true;
}
...
bool can_compress(const std::string& name) {
    Codec c;
    return find_codec(name, &c) && static_cast<bool>(c.compress);
}

bool can_decompress(const std::string& name) {
    Codec c;
    return find_codec(name, &c) && static_cast<bool>(c.decompress);
}
...
bool compress_bytes(const std::string& codec, const unsigned char* in, std::size_t n,
    ...
    Codec c;
    if (!find_codec(codec, &c) || !c.compress) { out.clear(); return false; }
...
bool decompress_bytes(const std::string& codec, const unsigned char* in, std::size_t n,
    ...
    Codec c;
    if (!find_codec(codec, &c) || !c.decompress) { out.clear(); return false; }
...
struct Reader {
    ...
    void need(std::size_t k) const {
        if (i > n || k > n - i) throw std::runtime_error("store file: the directory is truncated");
    }
...
struct CodecPlan {
    ...
    bool uses_codec(const DataStore& store) const {
        if (codec != kCodecNone) return true;
        for (int i = 0; i < store.n_columns(); ++i)
            if (for_column(store.column(i)) != kCodecNone) return true;
        for (const std::string& name : store.group_names())
            if (uses_codec(store.group(name))) return true;
        return false;
    }
};
...
struct Blobs {
    ...
    void read_stored(const BlobRef& r, std::uint64_t off, std::uint64_t n, void* into) const {
        if (n == 0) return;
        if (off > r.bytes || n > r.bytes - off ||
            r.offset > file_bytes || r.bytes > file_bytes - r.offset)
    ...
    void read_range(const BlobRef& r, std::uint64_t off, std::uint64_t n, void* into) const {
        ...
        if (r.transform != 0) {
            ...
            if ((width != 2 && width != 4 && width != 8) || raw.size() % width != 0)
                throw std::runtime_error("store file: invalid transform width");
...
struct OpenStore {
    ...
    OpenStore(const unsigned char* bytes, std::size_t n) : mem(bytes) {
        ...
        if (dir_offset < kHeaderBytes || dir_offset > file_bytes ||
            dir_bytes > file_bytes - dir_offset)
    ...
    OpenStore(const std::string& filename, std::uint64_t base_ = 0,
        ...
        if (base > whole || (region != 0 && region > whole - base))
            throw std::runtime_error(filename + " is shorter than the store in it");
...
bool plan_for(const StoreOptions& options, CodecPlan* plan, std::string* why) {
    ...
    if (!can_compress(options.codec)) {
        *why = "no " + options.codec + " encoder is available";
        return false;
    }
    plan->codec = id;
    return true;
...
struct File::Impl {
    ...
    bool seekhead_fits(int which, const Buf& body) {
        const std::uint64_t need = 6 + body.b.size();
        const std::uint64_t reserve = head_reserve[which];
        if (need > reserve || (need != reserve && reserve - need < kMinVoid))
    ...
    bool write_seekhead(int which, std::uint64_t gen) {
        ...
        if (!seekhead_fits(which, body)) return false;
...
std::uint64_t File::add_coded(const std::string& kind, const std::string& inner_encoding,
    ...
    if (!can_compress(codec)) { m.fail("no " + codec + " encoder is available"); return 0; }
    std::vector<unsigned char> packed;
...
bool File::update_coded(std::uint64_t uid, const unsigned char* data, std::size_t n,
    ...
    if (!can_compress(enc.codec)) return m.fail("no " + enc.codec + " encoder is available");
    std::vector<unsigned char> packed;
...
bool File::commit() {
    ...
    // Reject an already oversized directory before rewriting live metadata.
    // Placement can grow its entries, so write_seekhead checks the final size too.
    const int spare = 1 - m.live;
    if (!m.seekhead_fits(spare, m.build_seekhead(m.generation + 1))) return false;
