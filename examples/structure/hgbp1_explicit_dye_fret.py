...
# IMP.bff.<Name>; the sub-packages under IMP.bff.cgdye are only where the code
# lives (PRD-107). What the numbers mean and how they compare with the AV model
# is recorded in okf/validation/av_vs_rotamer.md (PRD-108).
...
eff = donor.fret_efficiencies(acceptor, donor="AlexaFluor 488", acceptor="AlexaFluor 594")
print(f"R0 = {eff['forster_radius_nm']:.2f} nm at <kappa2> = {eff['kappa2_avg']:.3f}; "
      f"E_static {eff['static']:.3f}, E_dynamic1 {eff['dynamic1']:.3f}, E_dynamic2 {eff['dynamic2']:.3f}")
...
positions = {"A481": IMP.bff.rotamer_ensemble_payload(donor), "A496": IMP.bff.rotamer_ensemble_payload(acceptor)}
...
print("wrote hgbp1_rotamer.fps.json:", {k: round(v["distance"], 1) for k, v in distances.items()})
# (AVNetworkRestraint scores AV positions only -- IMP.bff.fps_positions_for_docking() strips R1 ones)
...
IMP.bff.attach_dyes(protein, [(dye, "A", 481)], strip_site_sidechain=True)
sampler = IMP.bff.LangevinDyeSampler(protein, dye, str(get_structure_dir("alexa488_r48.mol2")), "A", 481,
