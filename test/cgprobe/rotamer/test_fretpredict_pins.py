...
The reference numbers in test/references/cgprobe_fretpredict_pins.json were
...
def test_rotamer_fret_matches_fretpredict(system_key: str, case_name: str, tmp_path: Path) -> None:
    ...
    if "R0_angstrom" in ref:
        # Angstrom on both sides. Every Forster radius in this package is
        # Angstrom, pins included, so no test has to carry a conversion --
        # a factor of ten living in an assertion is a factor of ten that can
        # be dropped from one. FRETpredict published this in nm, and the
        # pin's `R0_source` field records that so the citation survives the
        # unit change.
        assert float(np.ravel(fret.r0)[0]) == pytest.approx(
            ref["R0_angstrom"], abs=1e-3)
