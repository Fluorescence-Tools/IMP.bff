...
def _imp_kwargs(kwargs: dict[str, object]) -> dict[str, object]:
    """The same options in this package's unit.

    R0 is **Angström** here and nanometres in FRETpredict, so a fixed 5.5 nm
    is 55.0. Everything else means the same on both sides, which is the point
    of handing one dictionary to both.
    """
    out = dict(kwargs)
    if "r0" in out:
        out["r0"] = 10.0 * float(out["r0"])  # type: ignore[arg-type]
    return out
...
def _hsp90_pdb() -> Path:
    """Unpack the bundled Hsp90 fixture (test/cgprobe/rotamer/data) once per process."""
    import gzip
    import shutil
    import tempfile
    src = Path(__file__).resolve().parent / "data" / "openHsp90.pdb.gz"
    dst = Path(tempfile.gettempdir()) / "imp_bff_cgprobe_openHsp90.pdb"
    if not dst.exists():
