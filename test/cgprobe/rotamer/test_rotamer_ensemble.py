...
def test_matches_fretpredict_and_rotamer_fret(pair, hsp90, tmp_path):
    ...
    eff = d.pair_distribution_from_probes(a, "AlexaFluor 594", "AlexaFluor 568")
    assert eff.static_efficiency == pytest.approx(pins["Es"], abs=1e-5)
    ...
    # the pins are FRETpredict's, in nanometres; R0 is Angstrom here
    # Angstrom on both sides: the pin file stores what the API returns,
    ...
    assert float(fret.estatic_values[0]) == pytest.approx(eff.static_efficiency, abs=1e-9)
