...
class TestRotamerSampling(unittest.TestCase):
    def test_load_reference_rotamers(self):
        ...
        self.assertEqual(data.n_rotamers, 8)
    ...
    def test_apply_rotamer_coords(self):
        ...
        apply_coordinates(h, new_coords)
    ...
    def test_sample_rotamer_index(self):
        ...
        weights = [0.0, 1.0, 0.0]
        idx = sample_weighted_index(weights)
        self.assertEqual(idx, 1)

        weights = [0.1, 0.1, 0.8]
        samples = [sample_weighted_index(weights) for _ in range(100)]
