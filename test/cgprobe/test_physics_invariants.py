"""Physics invariants of cgprobe's FRET/rotamer machinery (PRD-107 stage 4).
...
from IMP.bff import transition_matrix_from_counts as _transition_matrix


def transition_matrix_from_counts(counts):
    """The C++ kernel takes the state count and answers flat."""
    counts = np.asarray(counts, dtype=float)
    n = int(counts.shape[0])
    return np.asarray(_transition_matrix(counts.ravel(), n)).reshape(n, n)
...
from IMP.bff import kb_kcal

KB_KCAL = kb_kcal()   # not a fourth copy of Boltzmann's constant
...
def test_kappa2_limits():
    ...
    assert kappa2_from_dipoles(z, z, r)[0, 0] == pytest.approx(4.0)   # collinear head-to-tail
    assert kappa2_from_dipoles(x, y, r)[0, 0] == pytest.approx(0.0)   # perpendicular, both ⟂ r
    assert kappa2_from_dipoles(x, x, r)[0, 0] == pytest.approx(1.0)   # parallel, ⟂ r
    assert kappa2_from_dipoles(x, z, r)[0, 0] == pytest.approx(0.0)   # one along r, one ⟂
...
def test_kappa2_isotropic_average_is_two_thirds():
    ...
    k2 = kappa2_from_dipoles(mu_d, mu_a, r)
    assert k2.shape == (n, n)
...
def test_forster_radius_scales_with_kappa2_to_the_sixth():
    r0_iso = forster_radius_from_spectra("AlexaFluor 488", "AlexaFluor 594", 2.0 / 3.0)
    r0_col = forster_radius_from_spectra("AlexaFluor 488", "AlexaFluor 594", 4.0)
    assert r0_col / r0_iso == pytest.approx(6.0 ** (1.0 / 6.0), rel=1e-12)
    assert forster_radius_from_spectra("AlexaFluor 488", "AlexaFluor 594", 0.0) == 0.0


@pytest.mark.parametrize("donor,acceptor,expected_angstrom", [
    ("AlexaFluor 488", "AlexaFluor 594", 56.87781310385014),
    ("AlexaFluor 594", "AlexaFluor 568", 47.37655494805884),
    ("AlexaFluor 488", "AlexaFluor 647", 53.78558552645125),
    ("AlexaFluor 546", "AlexaFluor 647", 68.58934378443619),
])
def test_forster_radius_pins(donor, acceptor, expected_angstrom):
    """R0 (nm) at κ² = 2/3 from the bundled spectra (recorded 2026-08-17)."""
    assert forster_radius_from_spectra(donor, acceptor, 2.0 / 3.0) == pytest.approx(expected_angstrom, abs=1e-9)


def test_forster_radius_matches_fretpredict_hsp90():
    # FRETpredict's Hsp90 case: <κ²> = 0.9692561424158548 -> R0 5.0425682 nm
    assert forster_radius_from_spectra("AlexaFluor 594", "AlexaFluor 568", 0.9692561424158548) == pytest.approx(50.425682, abs=1e-5)
...
def test_transition_matrix_rows_and_symmetric_counts():
    ...
    p2 = transition_matrix_from_counts([[0, 0], [1, 1]])
    assert p2[0, 0] == 1.0
...
def test_master_equation_slow_and_fast_exchange_limits():
    ...
    e_slow = fret_efficiency_exact_kinetic(np.eye(3), k, tau0, dt=1.0, weights=w)
    ...
    e_fast = fret_efficiency_exact_kinetic(p_fast, k, tau0, dt=1e-7, weights=w)
    k_avg = np.sum(w * k)
    assert e_fast == pytest.approx(k_avg / (k_avg + k_rad), abs=1e-6)
    # in between the two limits
    p_mid = 0.5 * np.eye(3) + 0.5 * p_fast
    e_mid = fret_efficiency_exact_kinetic(p_mid, k, tau0, dt=1.0, weights=w)
    ...
    assert fret_efficiency_exact_kinetic(p_fast, k, tau0, dt=1.0) == pytest.approx(
        fret_efficiency_exact_kinetic(p_fast, k, tau0, dt=1.0, weights=w), rel=1e-10)


def test_kronecker_pair_equals_explicit_product_space():
    p_d = transition_matrix_from_counts([[8, 2], [3, 7]])
    p_a = transition_matrix_from_counts([[5, 5], [1, 9]])
