...
from IMP.bff import ProbeAttachment, attach_probes, place_probe_from_coords
...
def _supported_reference_combinations():
    """Return list of (probe_name, pdb_path) for available reference dyes."""
...
def test_attach_all_reference_dyes_to_hgbp1_site481(combo_name, pdb_path, hgbp1_site481):
    """cgprobe placement equals the reference backbone-frame transform for every library."""
    model, protein, xyz = hgbp1_site481
    dye = IMP.atom.read_pdb(str(pdb_path), model, IMP.atom.AllPDBSelector())
    base = _coords(dye)
    assert base.shape[0] > 0

    # attach_probes moves the dye in place; the protein is not modified here
    # (strip_site_sidechain=False) so the module-scoped protein stays reusable.
    attach_probes(protein, [ProbeAttachment(dye, "A", 481)], strip_site_sidechain=False)
    placed = _coords(dye)
