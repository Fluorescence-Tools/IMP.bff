...
its own object model -- plus `IMP.bff.FitDataset` as a base, so that scoring,
...
`variance`, `residuals`, `objective`, `FitChiSquared.set_dataset` -- sees a
FitDataset.
"""
...
class DataCurveLike(Provenance, IMP.bff.FitDataset):
    ...
    def __init__(self, x, y, ey=None, filename=""):
        ...
        IMP.bff.FitDataset.__init__(self)
...
def test_it_is_a_dataset_and_still_itself():
    ...
    assert isinstance(c, IMP.bff.FitDataset)
...
def test_counts_without_errors_are_poisson_and_score_as_such():
    ...
    assert c.get_noise_family() == IMP.bff.FIT_NOISE_FAMILY_POISSON
    np.testing.assert_allclose(np.asarray(c.variance(mu)), mu)
    plain = IMP.bff.FitDataset()
    plain.set_values(list(y))
    plain.set_noise_family(IMP.bff.FIT_NOISE_FAMILY_POISSON)
...
def test_given_errors_it_stores_their_squares():
    ...
    assert c.get_noise_family() == IMP.bff.FIT_NOISE_FAMILY_STORED
...
def test_the_curve_can_be_combined_with_another_and_carry_the_uncertainty():
    ...
    vh.set_as_source("VH")
    g = 1.15
    magic = IMP.bff.FitDataset.add(vv, IMP.bff.FitDataset.affine(vh, 2.0 * g))
