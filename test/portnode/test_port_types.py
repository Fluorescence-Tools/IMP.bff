"""GraphPort value types: int, float, bool, and each of them as a vector.
...
read as a double -- `GraphPort(value=True).value` was `1.0`, which is only half a
type. This pins the six types, the coercion rules, and the two traps found
while adding them.

The codes are **not** free to change: `GraphSession` writes them into the chinet
...
from IMP.bff import (
    GRAPH_PORT_BOOL, GRAPH_PORT_BOOL_VECTOR, GRAPH_PORT_FLOAT, GRAPH_PORT_FLOAT_VECTOR, GRAPH_PORT_INT,
    GRAPH_PORT_INT_VECTOR, GraphPort, graph_port_value_type_element, graph_port_value_type_is_vector,
    graph_port_value_type_name, graph_port_value_type_of,
)


def test_the_chinet_codes_are_fixed():
    """0-3 are chinet's and are written into saved documents verbatim."""
    assert (GRAPH_PORT_INT, GRAPH_PORT_FLOAT, GRAPH_PORT_INT_VECTOR, GRAPH_PORT_FLOAT_VECTOR) == (0, 1, 2, 3)
    assert (GRAPH_PORT_BOOL, GRAPH_PORT_BOOL_VECTOR) == (4, 5)


@pytest.mark.parametrize("code,element,is_vector,name", [
    (GRAPH_PORT_INT, GRAPH_PORT_INT, False, "int"),
    (GRAPH_PORT_FLOAT, GRAPH_PORT_FLOAT, False, "float"),
    (GRAPH_PORT_INT_VECTOR, GRAPH_PORT_INT, True, "int[]"),
    (GRAPH_PORT_FLOAT_VECTOR, GRAPH_PORT_FLOAT, True, "float[]"),
    (GRAPH_PORT_BOOL, GRAPH_PORT_BOOL, False, "bool"),
    (GRAPH_PORT_BOOL_VECTOR, GRAPH_PORT_BOOL, True, "bool[]"),
])
def test_the_code_helpers_decompose_every_type(code, element, is_vector, name):
    assert graph_port_value_type_element(code) == element
    assert graph_port_value_type_is_vector(code) is is_vector
    assert graph_port_value_type_name(code) == name
    assert graph_port_value_type_of(element, is_vector) == code
...
def test_a_scalar_reads_back_as_its_own_type():
    assert GraphPort(value=12).value == 12
    assert isinstance(GraphPort(value=12).value, int)
    assert isinstance(GraphPort(value=1.5).value, float)
    assert GraphPort(value=True).value is True
    assert GraphPort(value=False).value is False
...
def test_a_bool_port_is_built_from_a_python_bool_not_an_int():
    ...
    for `__index__`.
    """
    assert GraphPort(value=True).get_value_type() == GRAPH_PORT_BOOL
    assert GraphPort(value=1).get_value_type() == GRAPH_PORT_INT
    assert GraphPort(value=np.bool_(True)).get_value_type() == GRAPH_PORT_BOOL
...
def test_a_bool_port_stays_bool_when_written_a_float():
    ...
    """
    p = GraphPort(value=True)
    p.value = 3.7
    assert p.value is True
    assert p.get_value_type() == GRAPH_PORT_BOOL
    p.value = 0
...
def test_an_int_port_accepts_a_float_and_coerces_it():
    ...
    """
    p = GraphPort(value=12)
    p.value = 1.5
    assert p.get_value_type() == GRAPH_PORT_INT
    assert p.value == 1
...
def test_an_integer_vector_keeps_its_type():
    ...
    """
    p = GraphPort(value=[1, 2, 3])
    assert p.get_value_type() == GRAPH_PORT_INT_VECTOR
    assert p.value.dtype == np.int64
    assert list(p.value) == [1, 2, 3]


def test_set_value_vector_int_is_named_not_overloaded():
    p = GraphPort(value=0)                       # an int port, not a float one
...
def test_an_integer_write_into_a_float_port_stays_float():
    ...
    """
    p = GraphPort(value=0.0)                     # float port
    p.set_value_vector_int([4, 5, 6])
    assert p.get_value_type() == GRAPH_PORT_FLOAT_VECTOR
    p.set_value_type(GRAPH_PORT_INT_VECTOR)
    assert p.get_value_type() == GRAPH_PORT_INT_VECTOR
...
def test_a_bool_vector_coerces_every_element():
    p = GraphPort(value=True)
    p.value = [1, 0, 2, -3]
    assert p.get_value_type() == GRAPH_PORT_BOOL_VECTOR
...
def test_a_vector_port_is_a_port_input_and_an_output():
    ...
    source = GraphPort(value=[1.0, 2.0, 3.0], is_output=True)
    sink = GraphPort(value=0.0)
...
# and both are pinned here. `GraphPort(value=2**53+1)` came back **typed float**,
...
def test_a_scalar_integer_survives_past_two_to_the_53(value):
    p = GraphPort(value=value)
    assert p.get_value_type() == GRAPH_PORT_INT, "a wide int must not land on double"
...
def test_a_wide_integer_survives_a_link():
    ...
    source = GraphPort(value=2 ** 60 + 7, is_output=True)
    sink = GraphPort(value=0)
    sink.set_link(source)
    assert sink.value == 2 ** 60 + 7


def test_a_wide_integer_survives_the_document():
    p = GraphPort(value=2 ** 60 + 7)
    restored = GraphPort()
    restored.read_json(p.get_json())
    assert restored.get_value_type() == GRAPH_PORT_INT
...
def test_a_numpy_bool_array_makes_a_bool_port_not_an_int_one():
    p = GraphPort(value=np.array([True, False, True]))
    assert p.get_value_type() == GRAPH_PORT_BOOL_VECTOR
    assert p.value.dtype == bool
...
def test_a_wide_integer_survives_a_saved_session(tmp_path):
    """The C++ writer and reader, not just GraphPort.get_json.

    `GraphSession` emitted through `get_value()` -- a double -- and read back
    ...
    """
    node = bff.GraphNode("n")
    node.add_input_port("count", GraphPort(value=2 ** 60 + 7))
    node.add_input_port("flag", GraphPort(value=True))
    node.add_input_port("ids", GraphPort(value=[2 ** 53 + 1, 3]))
    session = bff.GraphSession()
    session.add_node("n", node)
    path = str(tmp_path / "s.jsonl")
    session.save(path)

    restored = bff.GraphSession.load(path).get_node("n")
...
@pytest.mark.parametrize("declared,written,expected_code,expected", [
    (GRAPH_PORT_INT, 1.5, GRAPH_PORT_INT, 1),
    (GRAPH_PORT_INT, True, GRAPH_PORT_INT, 1),
    (GRAPH_PORT_FLOAT, 3, GRAPH_PORT_FLOAT, 3.0),
    (GRAPH_PORT_FLOAT, True, GRAPH_PORT_FLOAT, 1.0),
    (GRAPH_PORT_BOOL, 3.7, GRAPH_PORT_BOOL, True),
    (GRAPH_PORT_BOOL, 0, GRAPH_PORT_BOOL, False),
])
...
@pytest.mark.parametrize("declared,vector_code", [
    (GRAPH_PORT_INT, GRAPH_PORT_INT_VECTOR),
    (GRAPH_PORT_FLOAT, GRAPH_PORT_FLOAT_VECTOR),
    (GRAPH_PORT_BOOL, GRAPH_PORT_BOOL_VECTOR),
])
...
def test_a_bare_port_defaults_to_float():
    ...
    """
    p = GraphPort()
    assert p.get_value_type() == GRAPH_PORT_FLOAT
    p.value = [1.5, 2.5]
...
def test_set_value_type_is_the_only_way_to_change_a_declared_type():
    ...
    p = GraphPort(value=5)
    p.value = 9.9
    assert p.get_value_type() == GRAPH_PORT_INT and p.value == 9
    p.set_value_type(GRAPH_PORT_FLOAT)
    p.value = 9.9
    assert p.value == 9.9
...
def test_an_integer_port_reads_as_its_value_not_its_bit_pattern():
    ...
    assert list(np.asarray(GraphPort(value=[5, 6]).get_value_view())) == [5.0, 6.0]
...
def test_the_double_view_of_a_linked_integer_port_is_the_source():
    source = GraphPort(value=7, is_output=True)
    sink = GraphPort(value=0.0)
    sink.set_link(source)
...
def test_a_wide_integer_survives_being_pushed_to_a_follower():
    ...
    """
    source = GraphPort(value=0, is_output=True)
    follower = GraphPort(value=0)
...
def test_a_bool_scalar_round_trips_through_json(value):
    ...
    assert restored.get_value_type() == GRAPH_PORT_BOOL
...
def test_a_bool_vector_round_trips_through_json():
    ...
    restored = GraphPort()
    restored.read_json(p.get_json())
    assert restored.get_value_type() == GRAPH_PORT_BOOL_VECTOR
    assert list(restored.value) == [True, False, True]


@pytest.mark.parametrize("value,code", [
    (7, GRAPH_PORT_INT), (1.25, GRAPH_PORT_FLOAT), ([1, 2], GRAPH_PORT_INT_VECTOR),
    # A float vector built by the *constructor* reports the scalar float code,
    # not GRAPH_PORT_FLOAT_VECTOR. That is chinet's quirk and it is load-bearing --
