...
def test_attached_dye_sampler_hgbp1_481():
    ...
    for integrator in ("md", "bd"):
        ...
        s = LangevinDyeSampler(prot, dye, str(get_structure_dir("alexa488_r48.mol2")), "A", 481, integrator=integrator, seed=1)
        assert len(s.fixed) == 4 and len(s.mobile) == len(s.dye_particles) - 4
