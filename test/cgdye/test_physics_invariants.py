...
from IMP.bff.analysis.kinetic_fret import (
...
def test_forster_radius_pins(donor, acceptor, expected_nm):
    ...
    assert forster_radius_from_spectra(donor, acceptor, 2.0 / 3.0) == pytest.approx(expected_nm, abs=1e-9)


def test_forster_radius_matches_fretpredict_hsp90():
    # FRETpredict's Hsp90 case: <κ²> = 0.9692561424158548 -> R0 5.0425682 nm
    assert forster_radius_from_spectra("AlexaFluor 594", "AlexaFluor 568", 0.9692561424158548) == pytest.approx(5.0425682, abs=1e-6)
...
def test_transition_matrix_rows_and_symmetric_counts():
    ...
    p = rotamer_transition_matrix(counts)
    ...
    p2 = rotamer_transition_matrix([[0, 0], [1, 1]])
    assert p2[0, 0] == 1.0
...
def test_kronecker_pair_equals_explicit_product_space():
    p_d = rotamer_transition_matrix([[8, 2], [3, 7]])
    p_a = rotamer_transition_matrix([[5, 5], [1, 9]])
