...
Found while writing it: `labelizer_pair_score_negative_control` is exercised by
...
def test_the_standard_residues_are_the_twenty():
    names = list(bff.labelizer_standard_residues())
    assert len(names) == 20
...
def test_the_one_letter_code_round_trips_and_refuses_the_unknown():
    for three, one in (("ALA", "A"), ("TRP", "W"), ("GLY", "G"), ("CYS", "C")):
        assert bff.labelizer_one_letter(three) == one
    ...
    letters = [bff.labelizer_one_letter(r) for r in bff.labelizer_standard_residues()]
    assert len(set(letters)) == 20
    # Anything else is X, not a guess and not a crash.
    for unknown in ("MSE", "HOH", "XYZ", "", "A"):
        assert bff.labelizer_one_letter(unknown) == "X"


def test_a_categorical_lookup_refuses_a_key_it_does_not_have():
    """`labelizer_lookup` snaps to the nearest bin; `labelizer_lookup_key` must not.
    ...
    an invention.
    """
    table = bff.labelizer_load_table("C_CR1_Name")
    assert bff.labelizer_lookup_key(table, "C") == pytest.approx(3.8641, abs=1e-3)
    assert bff.labelizer_lookup_key(table, "W") == pytest.approx(0.2093, abs=1e-3)
    with pytest.raises(Exception) as excinfo:
        bff.labelizer_lookup_key(table, "Z")
...
def test_every_tag_has_a_score_type_and_an_unknown_tag_raises():
    for tag in ("cs", "se", "ss", "ce", "tp", "cr", "me", "combined"):
        assert bff.labelizer_score_type(tag)
    with pytest.raises(Exception):
        bff.labelizer_score_type("zz")
...
def test_residue_sasa_is_an_area_and_tracks_exposure(structure):
    """`labelizer_residue_sasa` is the unnormalised half of the RSA, and is exported
    because an area is a useful quantity on its own."""
    area = np.asarray(bff.labelizer_residue_sasa(structure, 1.4, 200))
    ...
    rsa = np.asarray(bff.labelizer_relative_solvent_accessibility(
        structure, bff.LABELIZER_MAXASA_WILKE, 1.4, 200))
    maxasa = dict(bff.labelizer_max_asa(bff.LABELIZER_MAXASA_WILKE))
...
def test_the_alpha_cone_places_the_dye_outward_and_within_reach(structure):
    ...
    checked = 0
    for i, residue in enumerate(structure.residues):
        position = bff.labelizer_alpha_cone_mean_position(structure, i, options)
        if len(position) != 3:
            continue
        position = np.asarray(position)
        cbeta = np.asarray(bff.labelizer_cbeta_position(structure, i))
        assert len(cbeta) == 3
...
def test_the_dye_model_selects_which_position_is_returned(structure):
    """`labelizer_probe_mean_position` dispatches on the option; the three models must
    ...
    for model in (bff.PROBE_MODEL_CBETA, bff.PROBE_MODEL_ALPHA_CONE,
        ...
        got = bff.labelizer_probe_mean_position(structure, pdb, site, options)
...
def test_an_out_of_range_site_returns_nothing_rather_than_guessing(structure):
    ...
    for bad in (-1, len(structure.residues), 10 ** 6):
        assert len(bff.labelizer_probe_mean_position(structure, _path("1DDB-39.pdb"),
                                            bad, options)) == 0
        assert len(bff.labelizer_cbeta_position(structure, bad)) == 0
...
def test_the_two_conformation_score_rewards_a_change_in_efficiency():
    ...
    r0 = 52.0
    assert bff.labelizer_pair_score_double(1.0, r0, r0, r0) == pytest.approx(0.0)
    # Symmetric in its two distances -- which conformation is "first" is not
    # a property of the pair.
    assert bff.labelizer_pair_score_double(1.0, 40.0, 70.0, r0) == pytest.approx(
        bff.labelizer_pair_score_double(1.0, 70.0, 40.0, r0))
    # A change spanning R0 beats the same-sized change far from it.
    near = bff.labelizer_pair_score_double(1.0, 45.0, 60.0, r0)
    far = bff.labelizer_pair_score_double(1.0, 130.0, 145.0, r0)
    assert near > far
...
def test_the_negative_control_rewards_a_pair_that_does_not_move():
    ...
    """
    r0 = 52.0
    still = bff.labelizer_pair_score_negative_control(1.0, r0, r0, r0)
    ...
    moved = bff.labelizer_pair_score_negative_control(1.0, 40.0, 70.0, r0)
    assert moved == pytest.approx(0.0)

    # Unmoving but at an uninformative distance scores below the ideal.
    far = bff.labelizer_pair_score_negative_control(1.0, 3 * r0, 3 * r0, r0)
    assert 0.0 <= far < still

    # And it scales with the joined label score like the other two.
    assert bff.labelizer_pair_score_negative_control(0.5, r0, r0, r0) == \
...
def test_the_settings_come_back_out_of_a_container(tmp_path):
    """`labelizer_read_pto_settings` is what makes a run reproducible from the file
    ...
    scores = bff.labelizer_score_structure(pdb, model, options,
                                    _path("1DDB-39_cs.pdb"))
    settings = bff.labelizer_settings_json(model, options, fret,
                                    _path("1DDB-39_cs.pdb"))
    out = str(tmp_path / "s.mmfdb.pto")
    bff.labelizer_write_pto(out, pdb, scores, bff.LabelizerPairScoreList(), settings)

    back = json.loads(bff.labelizer_read_pto_settings(out))
