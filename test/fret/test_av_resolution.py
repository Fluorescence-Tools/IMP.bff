...
``compute_av`` takes a position definition in the fps dialect and honours its AV
...
particle from the ``disc_step`` argument (``fret/av.py``), so a caller who states
...
_ARGS = (np.zeros((1, 4)), np.zeros(3), 20.0, 0.5, (3.5, 3.5, 3.5))
...
def test_disc_step_sets_the_grid_spacing(pdb_path, disc_step):
    av = IMP.bff.compute_av(*_ARGS, disc_step=disc_step,
                            pdb_path=pdb_path, source_info=dict(_SOURCE))
    assert av.grid_step == pytest.approx(disc_step)


def test_a_coarser_grid_is_a_smaller_array(pdb_path):
    fine = IMP.bff.compute_av(*_ARGS, disc_step=1.5, pdb_path=pdb_path,
                              source_info=dict(_SOURCE))
    coarse = IMP.bff.compute_av(*_ARGS, disc_step=2.5, pdb_path=pdb_path,
                                source_info=dict(_SOURCE))
...
def test_an_agreeing_declaration_is_accepted(pdb_path):
    av = IMP.bff.compute_av(
        *_ARGS, disc_step=2.0, pdb_path=pdb_path,
        source_info=dict(_SOURCE, simulation_grid_resolution=2.0))
...
def test_a_disagreeing_declaration_raises(pdb_path):
    with pytest.raises(ValueError, match="simulation_grid_resolution"):
        IMP.bff.compute_av(
            *_ARGS, disc_step=1.5, pdb_path=pdb_path,
            source_info=dict(_SOURCE, simulation_grid_resolution=2.5))
