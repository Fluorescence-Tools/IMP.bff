/**
 * \file ProbeAccessibleVolumeDecorator.cpp
...
static std::vector<double> av_cloud(const ProbeAccessibleVolumeDecorator& av);




void ProbeAccessibleVolumeDecorator::show(std::ostream &out) const {
  out << "(" << IMP::algebra::commas_io(get_parameter()) << ")";
}


double av_distance(
        const IMP::bff::ProbeAccessibleVolumeDecorator& av1,
        const IMP::bff::ProbeAccessibleVolumeDecorator& av2,
...
IMP::bff::PathMap* ProbeAccessibleVolumeDecorator::get_map() const{
    // get_map needs to be const
    if(av_map_ == nullptr){
        // cast away const to init map ¯\_(ツ)_/¯
        ProbeAccessibleVolumeDecorator* ptr = (ProbeAccessibleVolumeDecorator*)(this);
        ptr->init_path_map();
        ptr->resample();
    }
    return av_map_.get();
...
internal::AVLatticeState &ProbeAccessibleVolumeDecorator::get_state(){
    if(!state_){
...
void ProbeAccessibleVolumeDecorator::refresh_cloud_soa() const{
    auto map = get_map();   // builds and resamples on first use
    auto &st = const_cast<ProbeAccessibleVolumeDecorator*>(this)->get_state();
...
const std::vector<IMP::algebra::Vector4D> &ProbeAccessibleVolumeDecorator::get_cloud() const{
    refresh_cloud_soa();
    auto &st = const_cast<ProbeAccessibleVolumeDecorator*>(this)->get_state();
...
IMP::algebra::Vector3D ProbeAccessibleVolumeDecorator::get_mean_position(bool include_source) const{
...
IMP::Particle* ProbeAccessibleVolumeDecorator::get_source() const{
    return get_model()->get_particle(get_particle_index(0));
}

IMP::algebra::Vector3D ProbeAccessibleVolumeDecorator::get_source_coordinates() const{
    auto xyz = IMP::core::XYZ(get_source());
    return xyz.get_coordinates();
}

IntKey ProbeAccessibleVolumeDecorator::get_space_fixed_key(){
    static const IntKey k("av_space_fixed");
    return k;
}

bool ProbeAccessibleVolumeDecorator::get_space_fixed() const{
...
void ProbeAccessibleVolumeDecorator::set_space_fixed(bool tf){
    if(get_model()->get_has_attribute(get_space_fixed_key(), get_particle_index())){
...
IntKey ProbeAccessibleVolumeDecorator::get_search_grid_factor_key(){
    static const IntKey k("av_search_grid_factor");
    return k;
}

int ProbeAccessibleVolumeDecorator::get_search_grid_factor() const{
...
void ProbeAccessibleVolumeDecorator::set_search_grid_factor(int f){
...
IntKey ProbeAccessibleVolumeDecorator::get_search_stencil_key(){
    static const IntKey k("av_search_stencil");
    return k;
}

int ProbeAccessibleVolumeDecorator::get_search_stencil() const{
...
void ProbeAccessibleVolumeDecorator::set_search_stencil(int stencil){
...
IntKey ProbeAccessibleVolumeDecorator::get_compensate_stencil_key(){
    static const IntKey k("av_compensate_stencil");
    return k;
}

bool ProbeAccessibleVolumeDecorator::get_compensate_stencil() const{
...
void ProbeAccessibleVolumeDecorator::set_compensate_stencil(bool tf){
    int v = tf ? 1 : 0;
    if(get_model()->get_has_attribute(get_compensate_stencil_key(), get_particle_index())){
...
IntKey ProbeAccessibleVolumeDecorator::get_radii_source_key(){
    static const IntKey k("av_radii_source");
    return k;
}

std::string ProbeAccessibleVolumeDecorator::get_radii_source() const{
...
void ProbeAccessibleVolumeDecorator::set_radii_source(std::string source){
...
IntKey ProbeAccessibleVolumeDecorator::get_search_mode_key(){
    static const IntKey k("av_search_mode");
    return k;
}

std::string ProbeAccessibleVolumeDecorator::get_search_mode() const{
...
void ProbeAccessibleVolumeDecorator::set_search_mode(std::string mode){
    int v;
...
void ProbeAccessibleVolumeDecorator::set_occupancy_registry(ProbeAccessibleVolumeOccupancyRegistry *registry){
...
ProbeAccessibleVolumeOccupancyRegistry *ProbeAccessibleVolumeDecorator::get_occupancy_registry() const{
    if(!state_) return nullptr;
    return state_->registry.get();
}

std::vector<int> ProbeAccessibleVolumeDecorator::get_lattice_window() const{
    if(!state_ || !state_->have_window) return {};
    return {state_->k0[0], state_->k0[1], state_->k0[2], state_->n};
}

long ProbeAccessibleVolumeDecorator::get_number_of_skips() const{ return state_ ? state_->n_skip : 0; }
long ProbeAccessibleVolumeDecorator::get_number_of_local_updates() const{ return state_ ? state_->n_local : 0; }
long ProbeAccessibleVolumeDecorator::get_number_of_full_updates() const{ return state_ ? state_->n_full : 0; }
long ProbeAccessibleVolumeDecorator::get_number_of_rolls() const{ return state_ ? state_->n_roll : 0; }
unsigned long ProbeAccessibleVolumeDecorator::get_result_generation() const{
    return state_ ? state_->result_generation : 0;
}

void ProbeAccessibleVolumeDecorator::prepare_lattice_window(){
...
void ProbeAccessibleVolumeDecorator::init_path_map(){
...
void ProbeAccessibleVolumeDecorator::resample(bool shift_xyz, bool force_full){
    if(av_map_ == nullptr){
...
void ProbeAccessibleVolumeDecorator::resample_legacy(bool shift_xyz){
    auto map = get_map();
...
void ProbeAccessibleVolumeDecorator::resample_prepare(bool shift_xyz, bool force_full){
    if(av_map_ == nullptr){
        init_path_map();
    }
    if(get_space_fixed()){
...
void ProbeAccessibleVolumeDecorator::resample_compute(){
    if(state_ && state_->pending) resample_lattice_compute(false);
}

void ProbeAccessibleVolumeDecorator::resample_compute_search(){
    if(state_ && state_->pending) resample_lattice_compute(true);
}

void ProbeAccessibleVolumeDecorator::resample_compute_carve(){
    if(state_ && state_->pending) resample_lattice_compute_carve();
}

void ProbeAccessibleVolumeDecorator::resample_finish(){
    if(state_ && state_->pending) resample_lattice_finish();
}

void ProbeAccessibleVolumeDecorator::resample_lattice(bool shift_xyz, bool force_full){
    resample_lattice_prepare(shift_xyz, force_full);
    resample_lattice_compute(false);
    resample_lattice_finish();
}

void ProbeAccessibleVolumeDecorator::resample_lattice_prepare(bool shift_xyz, bool force_full){
    auto map = get_map();
    auto &st = get_state();
    st.pending = false;
    ...
    ProbeAccessibleVolumeOccupancyMap *occ1; ProbeAccessibleVolumeOccupancyMap *occ2;
    ...
    if(st.registry){
        ...
        for(size_t i = 1; i < dye_radii.size(); i++){
            ProbeAccessibleVolumeOccupancyMap *o = st.registry->get_map(h, dye_radii[i]);
        ...
        if(!st.private1 || st.private1->get_extra_radius() != extra1
            ...
            st.private1 = new ProbeAccessibleVolumeOccupancyMap(h, extra1, ps);
        ...
        if(!st.private2 || st.private2->get_extra_radius() != extra2
            ...
            st.private2 = new ProbeAccessibleVolumeOccupancyMap(h, extra2, ps);
        ...
        for(size_t i = 1; i < dye_radii.size(); i++){
            IMP::Pointer<ProbeAccessibleVolumeOccupancyMap> &pm = st.private_dye[i - 1];
            if(!pm || pm->get_extra_radius() != dye_radii[i]
                   || pm->get_spacing() != h){
                pm = new ProbeAccessibleVolumeOccupancyMap(h, dye_radii[i], ps);
...
void ProbeAccessibleVolumeDecorator::resample_lattice_compute(bool split_stages){
    auto &st = *state_;
...
void ProbeAccessibleVolumeDecorator::resample_lattice_compute_carve(){
    auto &st = *state_;
...
void ProbeAccessibleVolumeDecorator::resample_lattice_finish(){
    auto &st = *state_;
...
void ProbeAccessibleVolumeDecorator::set_av_parameter(const std::string &json_text){
...
IMP::bff::PathMapHeader ProbeAccessibleVolumeDecorator::create_path_map_header(){
...
void quadrature_points(
        const ProbeAccessibleVolumeDecorator &av, int k,
        const std::vector<IMP::algebra::Vector4D> *&points,
        const std::vector<std::array<double, 6> > *&moments){
    auto &st = const_cast<ProbeAccessibleVolumeDecorator&>(av).get_state();
...
void ProbeAccessibleVolumeDecorator::prepare_quadrature(int k) const{
    const std::vector<IMP::algebra::Vector4D> *pts;
    const std::vector<std::array<double, 6> > *mom;
    quadrature_points(*this, k, pts, mom);
}

std::vector<double> ProbeAccessibleVolumeDecorator::get_quadrature_points(int k) const{
    const std::vector<IMP::algebra::Vector4D> *pts;
    const std::vector<std::array<double, 6> > *mom;
    quadrature_points(*this, k, pts, mom);
...
double av_distance_quadrature(
        const ProbeAccessibleVolumeDecorator& av1,
        const ProbeAccessibleVolumeDecorator& av2,
        double forster_radius,
        int distance_type,
        int quad_k
){
...
std::vector<double> sample_points(const ProbeAccessibleVolumeDecorator& av, int n_samples){
    auto m = av.get_map();
...
void write_av(const ProbeAccessibleVolumeDecorator& av, const std::string& path) {
...
double rmp_from_measurement(const ProbeAccessibleVolumeDecorator& a, const ProbeAccessibleVolumeDecorator& b,
...
std::vector<double> rmp_flat_bottom_bounds(
        const ProbeAccessibleVolumeDecorator& a, const ProbeAccessibleVolumeDecorator& b, const AVPairDistanceMeasurement& measurement,
        double accuracy, int n_samples, int seed) {
...
double av_overlap(const ProbeAccessibleVolumeDecorator& av, const IMP::atom::Hierarchy& hierarchy,
...
void av_random_points(const ProbeAccessibleVolumeDecorator& av, double** out_view, int* n_out_view,
                      int n_samples){
    internal::copy_to_view(sample_points(av, n_samples), out_view, n_out_view);
}

void av_random_distances(const ProbeAccessibleVolumeDecorator& av1, const ProbeAccessibleVolumeDecorator& av2,
...
// -------- the doors over an IMP::Model (formerly ProbeAccessibleVolumeBuilder.cpp) --------
...
// (ProbeAccessibleVolumeBuilder.cpp) and builds no Model.

ProbeAccessibleVolume resample_av(IMP::Model* model, IMP::Particle* source_particle,
    ...
    ProbeAccessibleVolumeDecorator::do_setup_particle(model, av_particle->get_index(),
    ...
    ProbeAccessibleVolumeDecorator av(model, av_particle->get_index());
    ...
    return ProbeAccessibleVolume(points, density, origin, header->get_spacing(), "",
                            attachment);
}

ProbeAccessibleVolume get_av_from_structure(
    ...
    ProbeAccessibleVolume av = resample_av(
...
ProbeAccessibleVolume av_from_position(const std::string& pdb_path,
...
std::map<std::string, ProbeAccessibleVolume> get_avs_for_structure(
    ...
    std::map<std::string, ProbeAccessibleVolume> out;
    ...
    for (auto it = positions.begin(); it != positions.end(); ++it) {
        ...
        if (found == NULL || n_found == 0) {
            out[name] = ProbeAccessibleVolume(std::vector<double>(),
                                         std::vector<double>(),
                                         std::vector<double>(), step, name);
        } else {
            std::free(found);
            ProbeAccessibleVolume av = av_from_position(path, p, step);
