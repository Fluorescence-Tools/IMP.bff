/**
 * \file IMPHierarchyBridge.cpp
 * \brief The Hierarchy and Particle overloads of the core's functions.
 *
 * Sections in the order of IMP/bff/IMPHierarchyBridge.h; each is marked with
...
IMP::atom::Hierarchy hierarchy_from_protein_frame(const ProteinFrame& frame,
    ...
    for (std::size_t i = 0; i < n; ++i) {
        ...
        // Atom::setup_particle already installs Mass; retain the frame's
        // explicit element mass rather than setting the decorator up twice.
        IMP::atom::Mass(p).set_mass(IMP::atom::get_element_table().get_mass(element));
