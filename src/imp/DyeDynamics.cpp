...
DyeSimulation::DyeSimulation(const ProteinFrame& protein, const ProteinFrame& dye,
...
std::string DyeSimulation::get_parameters() const { return impl_->write(); }

void DyeSimulation::set_parameters(const std::string& json) {
...
int DyeSimulation::get_n_atoms() const {
    return static_cast<int>(impl_->dynamics->get_atom_names().size());
}

void DyeSimulation::get_positions(double** out_view, int* n_out_view) const {
    impl_->dynamics->get_coordinates(out_view, n_out_view);
}

void DyeSimulation::set_positions(const std::vector<double>& xyz) {
...
double DyeSimulation::minimize(int n_steps) {
    return impl_->dynamics->minimize(n_steps);
}

void DyeSimulation::step(int n_steps) {
    // The frames are what run() is for; stepping keeps only the state.
    impl_->dynamics->run(n_steps, n_steps > 0 ? n_steps : 1);
}

SimulationTrajectory DyeSimulation::run(int n_steps, int write_every) {
    return impl_->dynamics->run(n_steps, write_every);
}

double DyeSimulation::get_potential_energy() const {
    return impl_->dynamics->get_energy();
}

std::vector<std::string> DyeSimulation::get_atom_names() const {
    return impl_->dynamics->get_atom_names();
}

int DyeSimulation::get_n_stripped() const { return impl_->n_stripped; }

ProteinFrame DyeSimulation::get_labelled_frame() const {
...
double DyeSimulation::kinetic_temperature(double kinetic_energy) const {
