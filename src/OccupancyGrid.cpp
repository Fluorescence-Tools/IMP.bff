...
OccupancyGrid::OccupancyGrid(
    ...
    snapshot_(spheres) {
    IMP_USAGE_CHECK(spacing > 0, "OccupancyGrid: spacing must be positive");
}

OccupancyGrid::OccupancyGrid(
        double spacing, double extra_radius,
        const std::vector<IMP::algebra::Vector4D> &spheres, std::string name
) : IMP::Object(name), spacing_(spacing), extra_radius_(extra_radius),
    snapshot_(std::make_shared<const std::vector<IMP::algebra::Vector4D> >(spheres)) {
    IMP_USAGE_CHECK(spacing > 0, "OccupancyGrid: spacing must be positive");
}

void OccupancyGrid::atom_reach(int lo[3], int hi[3]) const {
    for (int d = 0; d < 3; d++) {
        lo[d] = std::numeric_limits<int>::max();
        hi[d] = std::numeric_limits<int>::min();
    }
    for (size_t i = 0; i < get_number_of_spheres(); i++) {
...
bool OccupancyGrid::covers(const int lo[3], const int hi[3]) const {
...
void OccupancyGrid::set_extent(const int k0[3], const int n[3]) {
    for (int d = 0; d < 3; d++) {
        k0_[d] = k0[d];
        n_[d] = n[d];
    }
...
void OccupancyGrid::request_window(int kx, int ky, int kz,
                                    int nx, int ny, int nz) {
    IMP_USAGE_CHECK(!fixed_extent_,
                    "OccupancyGrid: request_window on a fixed-window map");
    int lo[3] = {kx, ky, kz};
...
void OccupancyGrid::set_window(int kx, int ky, int kz,
                                int nx, int ny, int nz) {
    fixed_extent_ = true;
    ...
    if (!same_shape || force_full_ || last_.size() != get_number_of_spheres()) {
        set_extent(k0, n);
        return;
    }
...
void OccupancyGrid::record_change(const int lo[3], const int hi[3]) {
...
void OccupancyGrid::full_raster() {
    std::fill(counts_.begin(), counts_.end(), 0);
    int lo[3] = {k0_[0], k0_[1], k0_[2]};
    int hi[3] = {k0_[0] + n_[0] - 1, k0_[1] + n_[1] - 1, k0_[2] + n_[2] - 1};
    last_.resize(get_number_of_spheres());
    for (size_t i = 0; i < get_number_of_spheres(); i++) {
        IMP::algebra::Vector3D c = coord(i);
        double r = radius(i);
...
int OccupancyGrid::begin_update(bool force_full) {
    pending_action_ = 0;
    ...
    if (!fixed_extent_) {
        ...
        int lo[3], hi[3];
        bool have = false;
        if (include_atom_reach_ && get_number_of_spheres() > 0) {
            atom_reach(lo, hi);
            have = true;
    ...
    if (force_full || force_full_ || last_.size() != get_number_of_spheres()) {
        pending_action_ = 2;
        pending_box_all_ = true;
        moved_last_ = (long) get_number_of_spheres();
    ...
    pending_action_ = (2 * pending_moved_.size() >= get_number_of_spheres()) ? 2 : 1;
    return pending_action_;
}

void OccupancyGrid::raster_slab(int z_lo, int z_hi) {
...
void OccupancyGrid::apply_local() {
    int lo[3] = {k0_[0], k0_[1], k0_[2]};
    int hi[3] = {k0_[0] + n_[0] - 1, k0_[1] + n_[1] - 1, k0_[2] + n_[2] - 1};
    for (size_t i : pending_moved_) {
        const IMP::algebra::Vector4D &l = last_[i];
...
bool OccupancyGrid::update(bool force_full) {
...
void OccupancyGrid::read_window(int kx, int ky, int kz,
...
void OccupancyGrid::read_window_counts(int kx, int ky, int kz,
...
void OccupancyGrid::read_window_strided(int kx, int ky, int kz,
