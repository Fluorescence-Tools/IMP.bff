...
//! The first non-empty string of two keys, as the Python `or` chain read them.
...
//! A value as the string the Python `str(v)` produced: strings verbatim,
//! numbers by their JSON spelling, null as empty.
...
//! A string field, whatever the JSON type. The Python bridge did `str(v)`, so
//! a numeric id (`"id": 0`) is the string "0", not an error.
...
DyeForceFieldSystem forcefield_system_from_json(const std::string& json) {
    nlohmann::json d = nlohmann::json::parse(json);
    if (!d.is_object()) {
        IMP_THROW("forcefield_system_from_json needs a JSON object",
                  ValueException);
    }
    DyeForceFieldSystem out(ffjson_str(d, "name"));
