 ...
 *
 *  See include/CommandLine.h for the why. The subs here are the ones whose subject
...
void traj2drot_convert(const std::string& src, const std::string& dst,
    ...
    write_probe_rotamer_drot(dst, data_of(xyz), static_cast<int>(xyz.size()), names, elements,
...
void traj2drot_bundle(const std::vector<std::string>& sources,
    ...
    const std::vector<std::string> listed = probe_rotamer_drot_catalog(out);
...
int command_line_main(const std::vector<std::string>& args) {
    ...
    app.add_subcommand("help", "print help for the compiled subs")
        ->callback([&] {
            ...
            for (const auto& sub : command_line_subcommands()) {
