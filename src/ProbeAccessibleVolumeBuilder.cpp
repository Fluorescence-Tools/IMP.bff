/**
 * \file ProbeAccessibleVolumeBuilder.cpp
...
/* The accessible-volume search over spheres: the decorator's lattice path
   (ProbeAccessibleVolumeDecorator::resample_lattice_*), straight through, with none of the per-handle
...
IMP::Pointer<PathMap> run_lattice_search(
    ...
    // the effective parameters, as ProbeAccessibleVolumeDecorator::get_effective_* derive them
    ...
    // 0. the window, and the header anchored on it (ProbeAccessibleVolumeDecorator::create_path_map_header)
    int k0[3]; int n;
...
ProbeAccessibleVolume get_av_lattice(
        const std::vector<IMP::algebra::Vector4D>& spheres,
        const IMP::algebra::Vector3D& source, double source_radius,
        double linker_length, double linker_width,
        double r1, double r2, double r3, double h,
    ...
    return ProbeAccessibleVolume(points, density, origin, gh->get_spacing(), "", attachment);
}
...
ProbeAccessibleVolume get_av(double* atoms_xyzr, int n_atoms, int n_cols,
                            const std::vector<double>& source_xyz,
                            double linker_length, double linker_width,
    ...
    ProbeAccessibleVolume av = get_av_lattice(
            spheres, source, source_radius, linker_length, linker_width,
...
ProbeAccessibleVolume get_av_from_pdb(
    ...
    ProbeAccessibleVolume av = get_av(atoms, n_atoms / 4, 4, source, linker_length, linker_width,
                                 r1, r2, r3, grid_resolution, allowed_sphere_radius,
                                 search_stencil);
    std::free(atoms);
    std::free(src);
