...
double DockingPositionUncertainty::get_rmsf_mean() const {
...
double DockingPositionUncertainty::get_rmsf_max() const {
    if (rmsf.empty()) return std::nan("");
    double best = rmsf[0];
...
double DockingPositionUncertainty::get_mobile_rmsf_mean() const {
    if (rmsf.empty()) return std::nan("");
    double total = 0.0;
    int n = 0;
...
void DockingPositionUncertainty::get_rmsf(double** out_view, int* n_out_view) const {
    internal::copy_to_view(rmsf, out_view, n_out_view);
}

void DockingPositionUncertainty::get_mean_coords(double** out_view,
                                          int* n_out_view) const {
    internal::copy_to_view(mean_coords, out_view, n_out_view);
}

DockingPositionUncertainty estimate_position_uncertainty(
        const std::vector<std::string>& pdb_paths,
        const std::vector<std::string>& fixed_chains) {
    DockingPositionUncertainty out;
...
void write_position_uncertainty_pdb(const DockingPositionUncertainty& uncertainty,
...
void write_position_uncertainty_csv(const DockingPositionUncertainty& uncertainty,
