/**
 * \file ProbeAccessibleVolume.cpp
...
ProbeAccessibleVolume::ProbeAccessibleVolume(
...
void ProbeAccessibleVolume::get_density(double** out_view, int* n_out_view) const {
    internal::copy_to_view(density_, out_view, n_out_view);
}

void ProbeAccessibleVolume::get_grid_origin(double** out_view,
                                       int* n_out_view) const {
    internal::copy_to_view(grid_origin_, out_view, n_out_view);
}

void ProbeAccessibleVolume::update_points() {
...
ACV::ACV(const std::vector<double>& points, const std::vector<double>& density,
         ...
         const std::string& position_name)
        : ProbeAccessibleVolume(points, density, grid_origin, grid_step,
                           position_name),
...
ACV ACV::from_accessible_volume(const ProbeAccessibleVolume& av,
                                const std::vector<double>& slow_centers,
...
void write_av(const ProbeAccessibleVolume& av, const std::string& path) {
...
const ProbeAccessibleVolume& LabelDistribution::get_accessible_volume() const {
    if (!computed_) {
        do_compute();
        computed_ = true;
    }
...
void ProbeDistributionNormal::do_compute() const {
    ...
    av_ = ProbeAccessibleVolume(points, std::vector<double>(), std::vector<double>(),
