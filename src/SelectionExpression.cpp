...
//! Whether every term of \p n has an IMP predicate to compile to.
bool compiles_natively(const NodePtr& n) {
    if (!n) return true;
    switch (n->type) {
        case ParseNode::BYRES:
        case ParseNode::BYCHAIN:
        case ParseNode::DISTANCE:
            return false;  // no IMP predicate expands or measures a selection
        case ParseNode::PROPERTY:
            switch (n->what) {
                case P_SEGI:
                case P_ALT:
                case P_INDEX:
                case P_ID:
                    return false;  // not on IMP's hierarchy
                default:
                    for (const ValueTerm& v : n->values) {
                        if (v.has_wildcard) return false;
                    }
                    return true;
            }
        case ParseNode::FLAG:
            // The residue-class flags are residue-type lists, which IMP has;
            // the rest are per-atom questions it does not.
            switch (n->what) {
                case F_ALL:
                case F_NONE:
                    return true;
        ...
        default:
            return compiles_natively(n->a) && compiles_natively(n->b);
    }
}

IMP::ParticleIndexes sorted_unique(IMP::ParticleIndexes pis) {
    std::sort(pis.begin(), pis.end());
    pis.erase(std::unique(pis.begin(), pis.end()), pis.end());
    return pis;
}

IMP::ParticleIndexes set_union(const IMP::ParticleIndexes& a,
                               const IMP::ParticleIndexes& b) {
    IMP::ParticleIndexes out(a);
    out.insert(out.end(), b.begin(), b.end());
    return sorted_unique(out);
}

IMP::ParticleIndexes intersection(const IMP::ParticleIndexes& a,
                                  const IMP::ParticleIndexes& b) {
...
IMP::ParticleIndexes difference(const IMP::ParticleIndexes& a,
                                const IMP::ParticleIndexes& b) {
    IMP::ParticleIndexes out;
...
//! Exactly the particles a node matched, evaluated over the atom table.
IMP::ParticleIndexes materialize(const NodePtr& n,
                                 const std::vector<SelectionAtom>& atoms,
                                 const IMP::atom::Hierarchies& leaves,
                                 IMP::Model* model) {
    std::vector<char> mask;
    evaluate_node(n, atoms, mask);
    IMP::ParticleIndexes pis;
    for (std::size_t i = 0; i < mask.size(); ++i) {
        if (mask[i]) pis.push_back(leaves[i].get_particle_index());
    }
    return sorted_unique(pis);
}

IMP::ParticleIndexes all_indexes(const IMP::atom::Hierarchies& leaves) {
    IMP::ParticleIndexes pis;
    pis.reserve(leaves.size());
    for (const IMP::atom::Hierarchy& leaf : leaves) {
        pis.push_back(leaf.get_particle_index());
    }
    return sorted_unique(pis);
}

//! The integers a numeric value list names, ranges expanded.
IMP::Ints value_ints(const std::vector<ValueTerm>& values) {
    IMP::Ints out;
    for (const ValueTerm& v : values) {
        if (v.is_range) {
            for (int i = v.lo; i <= v.hi; ++i) out.push_back(i);
        } else {
            int n = 0;
            if (as_int(v.text, n)) out.push_back(n);
        }
    }
    return out;
}

//! The particles a node selects, through IMP's predicates where they exist.
/*! IMP's own set operations (`set_intersection` and friends) require both
    sides to have been built on the *same* hierarchies, and a term with no IMP
    predicate has to be built on the particles it matched instead. So the leaf
    terms are IMP selections -- `set_chain_ids`, `set_residue_indexes`,
    `set_residue_types`, `set_atom_types`, `set_element` -- and the algebra
    over them is over the particle indexes they select. */
IMP::ParticleIndexes compile_node(const NodePtr& n, IMP::atom::Hierarchy hierarchy,
                                  const std::vector<SelectionAtom>& atoms,
                                  const IMP::atom::Hierarchies& leaves,
                                  IMP::Model* model) {
    if (!n) return IMP::ParticleIndexes();
    if (!compiles_natively(n)) return materialize(n, atoms, leaves, model);

    switch (n->type) {
        case ParseNode::FLAG:
            if (n->what == F_ALL) return all_indexes(leaves);
            return IMP::ParticleIndexes();

        case ParseNode::PROPERTY: {
            IMP::atom::Selection s(hierarchy);
            switch (n->what) {
                case P_CHAIN: {
                    IMP::Strings ids;
                    for (const ValueTerm& v : n->values) ids.push_back(v.text);
                    s.set_chain_ids(ids);
                    break;
                }
                case P_RESI:
                    s.set_residue_indexes(value_ints(n->values));
                    break;
                case P_RESN: {
                    IMP::atom::ResidueTypes types;
                    for (const ValueTerm& v : n->values) {
                        const std::string t = upper(v.text);
                        if (IMP::atom::ResidueType::get_key_exists(t)) {
                            types.push_back(IMP::atom::ResidueType(t));
                        }
                    }
                    if (types.empty()) return IMP::ParticleIndexes();
                    s.set_residue_types(types);
                    break;
                }
                case P_NAME: {
                    IMP::atom::AtomTypes types;
                    for (const ValueTerm& v : n->values) {
                        const std::string t = upper(v.text);
                        if (IMP::atom::AtomType::get_key_exists(t)) {
                            types.push_back(IMP::atom::AtomType(t));
                        }
                        if (IMP::atom::AtomType::get_key_exists("HET:" + t)) {
                            types.push_back(IMP::atom::AtomType("HET:" + t));
                        }
                    }
                    if (types.empty()) return IMP::ParticleIndexes();
                    s.set_atom_types(types);
                    break;
                }
                case P_ELEM: {
                    IMP::ParticleIndexes acc;
                    for (const ValueTerm& v : n->values) {
                        IMP::atom::Selection one(hierarchy);
                        one.set_element(IMP::atom::get_element_table().get_element(
                                upper(v.text)));
                        const IMP::ParticleIndexes got =
                                one.get_selected_particle_indexes();
                        acc.insert(acc.end(), got.begin(), got.end());
                    }
                    return sorted_unique(acc);
                }
                default:
                    return materialize(n, atoms, leaves, model);
            }
            return sorted_unique(s.get_selected_particle_indexes());
        }

        case ParseNode::NOT:
            return difference(all_indexes(leaves),
                              compile_node(n->a, hierarchy, atoms, leaves, model));
        case ParseNode::AND:
            return intersection(
                    compile_node(n->a, hierarchy, atoms, leaves, model),
                    compile_node(n->b, hierarchy, atoms, leaves, model));
        case ParseNode::OR:
            return set_union(
                    compile_node(n->a, hierarchy, atoms, leaves, model),
                    compile_node(n->b, hierarchy, atoms, leaves, model));
...
IMP::atom::Selection selection_from_expression(IMP::atom::Hierarchy hierarchy,
                                     const std::string& expression) {
    const SelectionExpression parsed(expression);
    const std::vector<SelectionAtom> atoms = selection_atoms(hierarchy);
    const IMP::atom::Hierarchies leaves = IMP::atom::get_leaves(hierarchy);
    IMP::Model* model = hierarchy.get_model();
    if (parsed.get_is_empty()) {
        return IMP::atom::Selection(model, IMP::ParticleIndexes());
    }
    return IMP::atom::Selection(
            model, compile_node(parsed.get_root(), hierarchy, atoms, leaves,
                                model));
}
