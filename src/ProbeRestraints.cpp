...
ProbeSite::ProbeSite(int residue_seq_number, std::string atom_name,
...
double SimpleProbeNetworkRestraint::evaluate() const {
    double total = 0.0;
...
std::map<std::string, double> SimpleProbeNetworkRestraint::get_model_distances()
        const {
...
DirectProbeRestraint::DirectProbeRestraint(double* xyz, int n_atoms,
...
double DirectProbeRestraint::evaluate() const {
    if (n_atoms_ == 0 || sites_.size() < 2) return 0.0;

    double total = 0.0;
    for (std::size_t i = 0; i < sites_.size(); ++i) {
        const ProbeSite& si = sites_[i];
        for (std::size_t j = i + 1; j < sites_.size(); ++j) {
            const ProbeSite& sj = sites_[j];
