/** \file IMP/bff/FPSRotamer.h
 * Copyright 2007-2026 IMP Inventors. All rights reserved.
 */
#ifndef IMPBFF_FPSROTAMER_H
#define IMPBFF_FPSROTAMER_H
...
struct IMPBFFEXPORT FPSRotamerPosition {
    std::string name;
    ...
    FPSRotamerPosition() : residue(0), atom_name("CA") {}

    IMP_SHOWABLE_INLINE(FPSRotamerPosition,
                        out << "RotamerPosition(" << name << ", " << chain
                            << residue << ")");
};
IMP_VALUES(FPSRotamerPosition, FPSRotamerPositions);

//! A FRET distance between two labelling positions of an fps.json file.
struct IMPBFFEXPORT FPSRotamerDistance {
    std::string name;
    ...
    IMP_SHOWABLE_INLINE(FPSRotamerDistance,
                        out << "RotamerDistance(" << name << ": "
                            << donor_position << " -> " << acceptor_position
                            << ")");
};
IMP_VALUES(FPSRotamerDistance, FPSRotamerDistances);

//! One distance of an fps.json file, with both its positions and the document.
struct IMPBFFEXPORT FPSRotamerSelection {
    FPSRotamerPosition donor, acceptor;
    FPSRotamerDistance distance;
    ...
    FPSRotamerSelection() : positions("{}"), distances("{}"), extra("{}") {}

    IMP_SHOWABLE_INLINE(FPSRotamerSelection,
                        out << "RotamerFpsSelection(" << distance.name << ")");
};
IMP_VALUES(FPSRotamerSelection, FPSRotamerSelections);

//! Parse one fps.json position entry.
/*! \param[in] name the entry's key
    \param[in] payload_json the entry */
IMPBFFEXPORT FPSRotamerPosition fps_rotamer_position_from_payload(
...
IMPBFFEXPORT FPSRotamerDistance fps_rotamer_distance_from_payload(
        const std::string& name, const std::string& payload_json,
...
*/
IMPBFFEXPORT FPSRotamerSelection read_fps_rotamer(
...
/*!
    ...
    word.
*/
IMPBFFEXPORT std::string fps_rotamer_position_payload(
...
IMPBFFEXPORT std::string fps_rotamer_positions_payload(
...
*/
IMPBFFEXPORT std::string fps_rotamer_distances_from_ensembles(
        const std::map<std::string, ProbeRotamerEnsemble>& ensembles,
...
*/
IMPBFFEXPORT std::map<std::string, ProbeRotamerEnsemble> fps_rotamer_ensembles(
...
*/
IMPBFFEXPORT void write_fps_rotamer(const std::string& path,
                                    const std::string& positions_json,
                                    const std::string& distances_json = "{}",
                                    const std::string& merge_into = "",
                                    bool validate = true);

//! Build a #IMP::bff::FRETRotamer from an fps.json distance.
