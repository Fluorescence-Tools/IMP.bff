...
//! The same propagation, as one matrix exponential instead of n_steps sweeps.
/*!
    diffusion_propagate() takes `n_steps` explicit Euler sweeps because forward
    Euler on a diffusion operator is stable only for \f$dt \le dg^2/(6D)\f$.
    This takes none: the operator is **constant over a run** (\p d, \p decay and
    \p bounds do not change), so the whole propagation is \f$p(t) = e^{-Lt}p_0\f$
    for one \f$L\f$, and it is **symmetrisable** (Smoluchowski is symmetric as
    it stands; Ito becomes so under \f$q = \sqrt{d}\,p\f$). A Lanczos projection
    onto \p krylov_dim vectors therefore answers *every* reported time at once.

    Measured at 64-128 operator applications where the stepping route needs
    four thousand -- 31-62x fewer -- and **more accurate**, not less:
    diffusion_propagate() carries a forward-Euler and a first-order Lie
    splitting error of about 2e-6 that this does not.
    See `okf/validation/larger_time_steps.md`.

    **This does not reproduce diffusion_propagate()'s numbers.** It solves the
    semi-discrete equation; the other solves its own discretisation of it. The
    two agree to about the stepping scheme's own error, which is the honest
    tolerance for a comparison between them.

    \param[in] cur,d,decay,bounds,ng,flux_form,n_steps,n_out as diffusion_propagate()
    \param[in] krylov_dim Krylov vectors to build; <= 0 selects 128. A smooth
               initial density needs 64, a point-like one 128. Cost is linear
               in it and the error falls superlinearly, so too large is cheap
               and too small is not.
    \param[out] out_fluorescence,n_out_fluorescence as diffusion_propagate()
    \param[out] out_view,n_out_view the final density; pass nullptr for both to
               skip it and halve the work -- the trace alone needs one pass,
               the density needs a second.

    \throws std::invalid_argument if \p flux_form is FLUX_ITO and \p d is zero
            anywhere in the domain: the change of variables that symmetrises
            that form is \f$\sqrt{d}\f$.
*/
IMPBFFEXPORT void diffusion_propagate_krylov(
        const std::vector<double>& cur,
        const std::vector<double>& d,
        const std::vector<double>& decay,
        const std::vector<double>& bounds,
...
//! diffusion_propagate_krylov() without the density: half the work.
/*!
    The Krylov route needs one pass over the recursion for the trace and a
    second, with the coefficients then known, to rebuild the density -- storing
    the basis instead would be 544 MB at ng = 81, m = 128. Calibration wants
    the trace and nothing else (every gradient in the quenching stack is a
    finite difference through one), so this is the entry that halves it.

    \param[in] cur,d,decay,bounds,ng,flux_form,n_steps,n_out,krylov_dim
               as diffusion_propagate_krylov()
    \param[out] out_fluorescence,n_out_fluorescence the surviving population at
               each reported step
*/
IMPBFFEXPORT void diffusion_trace_krylov(
        const std::vector<double>& cur,
        const std::vector<double>& d,
        const std::vector<double>& decay,
        const std::vector<double>& bounds,
