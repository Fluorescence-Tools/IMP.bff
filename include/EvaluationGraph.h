...
class IMPBFFEXPORT EvaluationGraph {
  ...
  */
  ...
  /*!
      **The nodes are not written.** A node's work is its callback, and a
      callback is code -- in this library's intended use, Python supplied by
      the caller. What round-trips is the naming: which label points at which
      port of which node, by name. Rebuilding the nodes is the caller's, and
      then #from_json re-attaches the labels to them.
  */
