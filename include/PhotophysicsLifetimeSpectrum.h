/**
 *  \file IMP/bff/PhotophysicsLifetimeSpectrum.h
 ...
 *
 */
#ifndef IMPBFF_PHOTOPHYSICSLIFETIMESPECTRUM_H
#define IMPBFF_PHOTOPHYSICSLIFETIMESPECTRUM_H

#include <IMP/bff/bff_config.h>
#include <IMP/bff/PhotophysicsInteractionTerms.h>
...
*/
class IMPBFFEXPORT PhotophysicsLifetimeSpectrum {
...
public:
    ...
    /*! One constructor, not two: an overloaded one cannot carry keyword
        ...
        template needs. */
    PhotophysicsLifetimeSpectrum(const std::vector<double>& amplitudes =
                             std::vector<double>(),
    ...
    PhotophysicsLifetimeSpectrum normalized() const;
    ...
    PhotophysicsLifetimeSpectrum coarse_grain(int n_bins = 128) const;

    IMP_SHOWABLE_INLINE(PhotophysicsLifetimeSpectrum,
                        out << "PhotophysicsLifetimeSpectrum(" << get_n_species()
                            << " species, <tau>_x = "
                            << get_species_averaged_lifetime() << " ns, "
                            << (exact_ ? "exact" : "approximate") << ")");
};
IMP_VALUES(PhotophysicsLifetimeSpectrum, PhotophysicsLifetimeSpectrums);
...
/*!
    ...
    \param[in] exact see PhotophysicsLifetimeSpectrum
*/
IMPBFFEXPORT PhotophysicsLifetimeSpectrum lifetime_spectrum_from_rates(
...
/*!
    ...
    \param[in] exact see PhotophysicsLifetimeSpectrum
*/
IMPBFFEXPORT PhotophysicsLifetimeSpectrum lifetime_spectrum_from_states(
...
IMPBFFEXPORT double fret_efficiency_from_lifetimes(
        const PhotophysicsLifetimeSpectrum& donor_only,
...
#endif //IMPBFF_PHOTOPHYSICSLIFETIMESPECTRUM_H
