/**
 *  \file IMP/bff/Session.h
 *  \brief Save and load node graphs in chinet's on-disk session format.
 *
 *  A Session is what a chisurf .csp project is: a named graph of nodes
 ...
 *  chisurf, on top of the phase-1 Port/Node runtime). Deliberately
...
//! A saved-and-loadable graph of nodes and free ports, chinet's Session.
/*!
...
*/
class IMPBFFEXPORT Session : public BaseObject {
 public:
  ...
  typedef std::vector<std::shared_ptr<Port> > PortList;
  ...
  void add_node(const std::string& key, std::shared_ptr<Node> node);
  //! All nodes by key.
  NodeMap get_nodes() const;
  //! The node under a key, or a null pointer.
  std::shared_ptr<Node> get_node(const std::string& key) const;
  ...
  */
  static std::shared_ptr<Session> load(const std::string& path);
