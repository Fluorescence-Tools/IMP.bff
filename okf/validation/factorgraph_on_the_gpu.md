...
## The shape it would take, and the prototype that already exists

Owner's framing, and it is the right one: **a factor graph is a neural network
whose transforms are more complicated.** A chain of nodes, each consuming its
predecessors' outputs; what crosses the boundary is small at both ends -- the
free parameters going in (kilobytes) and a scalar objective coming back (four
bytes). Everything large is intermediate, and intermediates never need to
leave the device.

That is not an analogy to a thing that would have to be invented. It is the
architecture `gpu/mlp.wgsl` and `mlp_forward` already implement: layer outputs
ping-pong between two device buffers, the weights are uploaded once, and one
`predict()` crosses once however deep the network. The generalisation from a
network to a graph is two changes, both bounded:

| from the MLP path | to a factor graph |
|---|---|
| a chain, so two buffers ping-ponged | an arbitrary DAG, so a topological order and a buffer allocator that frees an intermediate once its last consumer has run |
| one dense-layer kernel | a kernel per node, which for expression nodes means **emitting WGSL from the expression tree** rather than writing shaders by hand |

The second is the real work and it is a compiler, not a shader. The first is
ordinary.

## When it becomes worth doing

Not a matter of opinion; the measurements above give thresholds. The condition
is **work per synchronisation**, because that is the only fixed cost left once
the data is resident:

- below ~50 datasets × 4 096 points in one crossing, the CPU wins outright;
- at 200 datasets it is 9.1× with one evaluation per synchronisation and
  **58.6× with the eight a seven-parameter Levenberg-Marquardt step already
  offers**;
- at 2 000 datasets, 41× with a single evaluation per synchronisation.

So the trigger is: **more data and more complex graphs**, which is exactly
when it stops being worth ignoring. A global fit over a few hundred datasets
is already past the threshold; the current catalogue's handful is not.
