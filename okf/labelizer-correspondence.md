...
Conventions: everything ported carries the `ll_` / `Ll` prefix so the ported
...
|---|---|---|---|
| `labelizer.py` | 466 | `ll_score_structure`, `bin/imp_bff_labelizer` | orchestration; the `Labelizer` class does not survive as a class |
| `labeling_parameter.py` | 267 | `LlTable`, `ll_load_table`, `ll_lookup`, `ll_lookup_key`, `ll_parameter_scores` | the base class becomes a table plus a dispatch |
| `labeling_score.py` | 286 | `ll_labeling_score`, `LlParameter`, `ll_model_paper` | the weighted geometric mean |
| `cysteine_resemblance.py` | 85 | `ll_parameter_scores`, tag `cr` | pure lookup |
| `secondary_structure.py` | 158 | **`ll_dssp`** + tag `ss` | the DSSP *binary* became C++ |
| `solvent_exposure.py` | 163 | **`ll_residue_depth`**, `ll_relative_solvent_accessibility`, `ll_half_sphere_exposure` + tag `se` | the MSMS *binary* became C++ |
| `conservation_score.py` | 155 | `ll_read_consurf` + tag `cs` | import only, as in the reference |
| `methionin_exclusion.py` | 100 | `ll_parameter_scores`, tag `me` | |
| `tryptophan_proximity.py` | 234 | `ll_parameter_scores`, tag `tp` | ported *working*; raises in the reference |
| `charge_environment.py` | 351 | `ll_parameter_scores`, tag `ce` | ported *working*; raises in the reference |
| `fret_score.py` | 978 | `LabelizerFret.h` in full | the biggest module |
| `measurement_score.py` | 196 | `LlPairScore`, `ll_pair_scores*` | the CSV base class is gone |
...
| `pdbhelper.py` | 108 | `ll_read_structure` | over `read_pdb_records` |
| `auxiliary_functions.py` | 82 | **not ported** | MSMS binary discovery, plus dead code |
| `config.py` | 147 | `LlOptions`, `LlFretOptions` | constants become defaults on a struct |
| `__init__.py` | 65 | `ll_model_paper`, `ll_available_tables` | |

## Function by function, where it is not obvious

| reference | imp.bff |
|---|---|
| `Labelizer.calc_parameter_score()` (`labelizer.py:216`) | `ll_parameter_scores` |
| `Labelizer.calc_labeling_score()` (`:246`) | `ll_labeling_score` |
| `Labelizer.calc_fret_score()` (`:261`) | `ll_pair_scores` / `ll_pair_scores_two_states` |
| `FRETScore.calc_measurement_scores(chains_apo, chains_holo)` (`fret_score.py:479`) and the chain filter at `:728` | `LlFretOptions::donor_chain` / `::acceptor_chain`, plus `::chain_map` for two files that name a chain differently. **Ported 2026-09-07**; it was missing until then, and without it a homodimer cannot be screened at all — see `okf/log.md` 2026-09-07 (1). |
| `Labelizer.zip_files()` (`:423`) | `ll_write_pto` — one container, not a zip |
| `LabelingParameter.factory(tag)` (`labeling_parameter.py:56`) | a tag string on `LlParameter`; no class hierarchy |
| `LabelingParameter.calc_score_frequency(v)` (`:184`) | `ll_lookup` (numeric) / `ll_lookup_key` (categorical) |
| `LabelingParameter._load_logic(m)` (`:198`) | `ll_load_table`, cached on name |
| `LabelingParameter._save_pdb/_save_csv` (`:~230`) | **gone** — see *What did not come across* |
| `LabelingScore._labeling_score(...)` (`labeling_score.py:187`) | `ll_labeling_score`, weight as repeat count |
| `label_score_model_paper` (`labelizer.py:40`) | `ll_model_paper()` |
| `Labelizer.parameter_score_model` (`:72`) | the `table` field of each `LlParameter` |
| `FRETScore.joined_label_score(...)` (`fret_score.py:192`) | `ll_joined_label_score` |
| `FRETScore.calc_measurement_score(...)` (`:225`) | `ll_pair_score_double` |
| `FRETScore.calc_measurement_score_single(...)` (`:263`) | `ll_pair_score_single` |
| the negative control (`:243`) | `ll_pair_score_negative_control` — the reference computes and discards it |
| `FRETScore._fret_efficiency(r)` (`:260`) | `IMP::bff::fret_efficiency`, which already existed |
| `FRETScore._get_cb_position(res)` (`:317`) | `ll_cbeta_position` |
| `FRETScore._get_gly_cb_vector(...)` (`:293`) | folded into `ll_cbeta_position` |
| `FRETScore._calc_simple_mean_position(...)` (`:326`) | `ll_alpha_cone_mean_position` |
| `FRETScore._calc_single_av(...)` (`:404`) | `compute_av_from_structure` — the module's own AV |
| `FRETScore._calc_distances_single(...)` (`:641`) | `ll_pair_scores`, with the AVs cached per site |
| `FRETScore._calc_heat_map(...)` (`:920`) | `ll_cbeta_difference_map` |
...
| `pdbhelper.remove_hetatoms(model)` (`pdbhelper.py:16`) | the `protein_only` argument of `ll_read_structure` |
| `ChargeEnvironment.calc_global_charge()` (`charge_environment.py:160`) | `ll_global_charge` — net, positive and negative formal charge. Offered by the reference and consumed by nothing there. Its PQR variant, which reads a partial charge out of the **occupancy** column, is not ported: see the header. |
...
| reference | imp.bff |
|---|---|
| `LS_THRESHOLD = 0.5` | `LlFretOptions::label_score_threshold` |
| `AV_CALC_RADIUS = 20.0` | `LlFretOptions::alpha_cone_radius` |
| `COLLISION_RADIUS = 1.7` | `LF_COLLISION_RADIUS` (file-local in `LabelizerFret.cpp`) |
| `GS_PRECISE = 0.8` / `GS_FAST = 1.2` | `LlFretOptions::grid_resolution` |
...
| `max_bad_aa_distance = 6`, `min_bad_aa_solvent_exposure = 0.4` | `LlOptions::exclusion_distance`, `::exclusion_exposure` |
| `HSE_RADIUS = 13.0` | `LlOptions::hse_radius` |
...
* **`fluorophore.py`** — `ProbeLibrary.h` already models a dye and derives
  ...
  properties: they are AV parameters, and they live on `LlFretOptions` here and
...
| | reference | here |
|---|---|---|
| DSSP | external binary; **fails on macOS** (`secondary_structure.py:126`) | `ll_dssp`, native. Reproduces the binary exactly on 1DDB (195/195) |
| residue depth | MSMS binary; **does not execute** (32-bit ppc/i386) | `ll_residue_depth`, native. No bias, r = 0.976 vs the reference's output |
...
| a sequence gap | array-indexed turns bridge it silently | `ll_dssp` refuses to bridge; see `test_dssp_does_not_infer_a_peptide_bond_across_a_sequence_gap` |
| matching sites across two conformations | by residue number (`fret_score.py:512`), so chains collide on a multimer | by `(chain, seq_id)`, with `LlFretOptions::chain_map` when the names differ. The reference's behaviour is not selectable: it returns `distance_2 = 0` for every inter-chain pair, which is not a modelling choice but a defect |
| the Cbeta difference map on a multimer | indexed by residue number, every chain on the same row | `ll_cbeta_difference_map` takes a `chain` |
...
Three arithmetic differences are **selectable, not fixed** — `LlModel`,
...
|---|---|---|
| joined label score uses `prod ** 0.5`, not `prod ** (1/N)` | `fret_score.py:220` | `LL_MODEL_PUBLISHED` |
| a weight-0 term reading exactly `0.0` zeroes the whole score | `labeling_score.py:166` | `LL_MODEL_PUBLISHED` |
| the excluded amino acid is always MET whatever is configured | `labelizer.py:233` | `LL_MODEL_PUBLISHED` |
